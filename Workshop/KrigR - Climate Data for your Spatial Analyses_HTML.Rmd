...
Our current analyses suggest that `nmax = 120` is a very sensible choice, but we are working on an automatic feature for the identification of an approripate number of `nmax` straight from the data itself.
...
NDVI_Krig <- krigR(Variable = '2m_temperature',
                   ...
                   nmax = 20, # this is smaller than we recommend, but placed here to show some neat, localised uncertainty patterns
                   Dir = Dir.NDVI)
```
