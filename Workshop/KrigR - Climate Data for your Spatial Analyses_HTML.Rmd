...
```{r ClimExt, results = 'hide', fig.keep = 'all'}
...
```{r CovExt, results = 'hide', fig.keep = 'all'}
...
Pipe_Krig <- krigR(Variable = '2m_temperature',
                    ...
                    Dir = Dir.StatePipe)
Pipe_Krig
Plot_Krigs(Pipe_Krig, Date = c("02-01-1995", "03-01-1995", "04-01-1995"))
```
...
Shapes <- readOGR(Dir.Shapes, # where to look for the file
                  "ne_10m_admin_1_states_provinces", # the file name
                  verbose = FALSE) # we don't want an overview of the loaded data
Position <- which(Shapes$name_en == "Saxony") # find Saxony
State_Shp <- Shapes[Position,]
...
```{r ClimShape, results = 'hide', fig.keep = 'all'}
State_Raw <- download_ERA(
  Variable = '2m_temperature',
  DataSet = 'era5-land',
  DateStart = '1995-01-02',
  ...
  API_User = API_User,
  API_Key = API_Key)
Plot_Raw(State_Raw, Shp = State_Shp, Date = c("02-01-1995", "03-01-1995", "04-01-1995"))
```
...
```{r CovShape, results = 'hide', fig.keep = 'all'}
Covs_ls <- download_DEM(Train_ras = State_Raw,
                        Target_res = .02,
                        Shape = State_Shp,
                        Dir = Dir.StateShp,
...
Plot_Krigs(State_Krig, Shp = State_Shp, Date = c("02-01-1995", "03-01-1995", "04-01-1995"))
```
...
<!-- ## Kriging to match already available data sets   -->
<!-- Train_res set to a raster with gimms package using Era5 data across Scotland -->

<!-- ## Other variables and recommendations for downscaling -->
...
`krigR()` also supports any combination of ERA-family reanalysis, GMTED2010, third-party climate data, and third-party covariate data.
